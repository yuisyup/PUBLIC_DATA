...
class RegisterFromCsvUsecase(RegisterUsecaseProtocol, AbstractRegisterUsecase):
    ...
    def execute(
        ...
        # 2. DataFrameのFKを解決
        try: 
            fk_resolver = FkResolveProcessor(
                ModelRegistry(), DjangoModelPersister())
            df_resolved: pd.DataFrame = fk_resolver.resolve_foreign_keys(
                df=df_preprocessed,
                fk_specs=self._col_specs,
                issues=issues)
        ...
        # 登録ポリシークラスを取得
        try:
            register_policy_processor_instance: RegisterProcessorProtocol = PolicyProcessorFactory().get_register_processor_by_policy(
