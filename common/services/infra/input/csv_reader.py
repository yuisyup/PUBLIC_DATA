...
class CsvReader:
    ...
    def read_csv_to_dataframe(
        ...
        try:
            ...
            raise CsvParseError(
                message="文字コードがUTF-8ではありません。") from e

        except pd.errors.EmptyDataError:
            raise CsvParseError(
                message="CSVが空です。") from e

        except pd.errors.ParserError as e:
            raise CsvParseError(
                message="CSVの構造が壊れています。") from e

        except ValueError as e:
            raise CsvParseError(
                message="read_csvの引数設定に問題があります。") from e

        except OSError as e:
            raise CsvParseError(
                message="ファイルストリームが壊れているか閉じています。") from e

        except Exception as e:
            raise CsvParseError(
                message="その他の予期しないエラーが発生しました。") from e
