...
class InputDefinitionRepository:
    ...
    def lookup_column_defs(self, input_id: int):
        ...
        # カラム定義取得
        column_defs: QuerySet[MsInputColumnDef] = MsInputColumnDef.objects.filter(input_id=input_id).order_by('column_order')
    ...
    def get_input_def_choices_by_type(
        ...
        # id, 名前でのタプルリストを返却
        choices: List[tuple[int, str]] = [(str(ms.input_id), str(ms.input_name)) for ms in input_def_list]
