...
class FkResolveProcessor:
    ...
    def resolve_foreign_keys(
        ...
        for i, fk_spec in enumerate(fk_specs):
            ...
            except (FkResolveDefIncompleteError,
                    FkResolveLookupFieldsMismatchError) as e:
            ...
            try:
                ...
                target_model: models.Model = self._model_registry.resolve(fk_spec.fk_target_model)
            ...
            for i, row in df_resolved.iterrows():
                ...
                try:
                    lookup_kwargs: Dict[Any, Any] = Build.build_lookup_kwargs(row_dict, fk_spec)
                ...
                try:
                    # FK変換先インスタンスを取得
                    fk_model_instance = self._persister.get(target_model, **lookup_kwargs)
                ...
                # ★FKへの変換実施★
                df_resolved.at[i, fk_spec.model_target_field_name] = getattr(fk_model_instance, fk_spec.fk_target_field)
        ...
        drop_cols = [
            ...
            if fk_spec.is_lookup_only and fk_spec.model_target_field_name in df_resolved.columns
        ]
