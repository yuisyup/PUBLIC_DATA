    ...
    def create_dataframe(
        ...
        try:
            df_raw: pd.DataFrame = CsvReader().read_csv_to_dataframe(
                csv_file, input_def_spec.delimiter, input_def_spec.has_header
            )
        ...
        col_dict: Dict[int, str] = MappingColumn.map_def_column(specs=spec_input_list)

        # Dataframeのカラム名変換を実行
        df_mapped: pd.DataFrame | None = Replace.replace_column_name(
            col_dict=col_dict, df_raw=df_raw
        )
