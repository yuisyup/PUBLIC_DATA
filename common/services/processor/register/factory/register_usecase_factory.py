...
from common.exceptions.register_errors import (
    ...
    RegisterUsecaseClassNotFoundError,
)
...
from common.services.domain.register.dto import (
    InputDefRaw,
    InputDefSpec,
    FkResolveSpecInput,
    FkResolveSpec,
...
class RegisterUsecaseFactory:
    ...
    def get_register_usecase(self, input_id: int) -> RegisterUsecaseProtocol:
        """
        ...
        col_specs_raw: List[FkResolveSpecInput] = repo.lookup_column_defs_as_dto(
            input_id=input_id
        )
        if not col_specs_raw:
            raise ColumnDefinitionNotFoundError(
                context={"input_id": input_id, "csv_name": def_spec.input_name}
            )
        ...
        return register_usecase(def_spec=def_spec, col_specs=col_specs)
