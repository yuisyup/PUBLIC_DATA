...
class MsInputDef(models.Model):
    ...
    input_type = models.ForeignKey(MsInputType,on_delete=models.PROTECT, db_column='input_type_code')
    delimiter = models.CharField(max_length=5, null=True, blank=True)
    has_header = models.BooleanField(null=True, blank=True)
    register_policy = models.ForeignKey(MsRegisterPolicy,on_delete=models.PROTECT, db_column='policy_code')
...
class MsInputColumnDef(models.Model):
    ...
    def __str__(self):
        return str(self.input_id.input_name) + ' : ' + str(self.column_order) + ' : ' + str(self.display_name)
